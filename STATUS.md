...
1. MagicBlock validation lane is documented in `docs/MAGICBLOCK-PER-TEE-VALIDATION-2026-05-07.md`. Current proof is strong boundary evidence, not successful settlement. Latest artifacts: boundary submission `artifacts/per-happy/20260507-115112/` and post-router-patch rejection `artifacts/per-happy/20260507-115318/`. If Nissan wants a stronger MagicBlock prize push, implement docs-conformant MagicBlock permission/delegation CPI hooks for escrow PDA state next; otherwise use the current judge-safe wording in the runbook.
