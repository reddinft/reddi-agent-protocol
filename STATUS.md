...
**State:** 🟡 Quasar-first final demo realignment active via Issue #236 and PR #244. Phase 13 local Surfpool/devnet-prep loop is complete, but Nissan clarified the final goal is broader: move final Colosseum demos from Anchor-compiled proof to Quasar-compiled Solana program proof and visibly use MagicBlock/x402/Jupiter/OpenRouter/Surfpool ecosystem evidence. Devnet mutation remains approval-gated. Prior Issue #228 packet was Anchor-current-state readiness, but Nissan now requires hackathon demos to use Quasar-deployed Solana programs. Live research, real image generation, signing, deployment, wallet, and env mutations remain approval-gated.
...
3. Quasar cutover follows Issue #236, `docs/QUASAR-HACKATHON-CUTOVER-PLAN-2026-05-05.md`, and the refined staged playbook `docs/QUASAR-BDD-ITERATIVE-PLAYBOOK-2026-05-05.md`. Phase 1 inventory is scaffolded at `config/quasar/deployments.json` with `npm run check:quasar:deployments`. Candidate devnet Quasar program `VYCbMszux9seLK2aXFZMECMBFURvfuJLXsXPmJS5igW` is executable by read-only RPC check. Phase 2 guard is `npm run check:quasar:demo-readiness`; Phase 3 app/demo-agent target flag wiring merged via PR #239. Use `NEXT_PUBLIC_DEMO_PROGRAM_TARGET=quasar` / `HACKATHON_DEMO_TARGET=quasar` / `DEMO_PROGRAM_TARGET=quasar` for devnet Quasar target selection. Phase 5/6/7/8/9 is active in PR #244 on `feat/quasar-shared-instruction-builders-20260505`: `/register`, demo-agent registration, onboarding attestation construction, `/economic-demo` honesty UI, read/decode paths, reputation commit/reveal, and onboarding registration/confirm/dispute builders are Quasar-compatible. Phase 9 scoped the legacy full-flow/PER demo-agent script out of Quasar proof and added a fail-closed Quasar target guard. Runtime compatibility blockers reduced 6→4→1→0. Phase 10 added a Quasar CI/readiness guard. Phase 11 refreshed the judge packet/operator checklist/scoped proof doc; `submissionReady=true` now applies to the scoped Quasar proof boundary (not live PER/TEE). Phase 12 decision: scoped Quasar proof is locally complete; do not perform live devnet/PER/signing validation without explicit Nissan approval. Phase 13 local confidence/devnet-prep is complete: economic Surfpool rehearsal and critical Surfpool regression passed, read-only devnet PDA inspection found A/B/C registered under legacy Anchor and absent under Quasar, and `docs/QUASAR-DEVNET-VALIDATION-RUNBOOK-2026-05-06.md` defines the approval-gated next step. PR #244 head `f158acb5` is CI-green and mergeable (`mergeStateStatus=CLEAN`): Anchor/LiteSVM, BDD index, Quasar readiness, source conformance, and Vercel all passed. Next operational step: follow Phase 14 Quasar-first final demo alignment (`docs/QUASAR-FINAL-DEMO-ELIGIBILITY-MATRIX-2026-05-06.md`): treat scoped proof as fallback, register/read back A/B/C on Quasar devnet after explicit approval, and map MagicBlock/x402/Jupiter/OpenRouter/Surfpool evidence without overclaiming legacy Anchor paths. PER live validation still approval-gated/not claimed. Do not deploy, sign, mutate wallets, mutate env/Coolify/Vercel, or run paid/live specialist work without explicit approval.
...
- Local working tree: clean after pushing Phase 13 Surfpool/devnet-prep runbook + target-aware deregistration fix in PR #244 commit `f158acb5`.
