...
0. RAP MCP Bridge PR #254 clean-branch repair in progress: original PR branch accidentally inherited prior local Umbra/registration-fee commits. Rebuilt from `origin/main` as `feature/rap-mcp-bridge-clean`, cherry-picking RAP MCP Bridge only, plus `.npmrc` `legacy-peer-deps=true` because root `npm ci` fails on the existing Umbra sdk/web-zk-prover peer conflict without it. Next: validate clean branch, force-update PR #254, and monitor checks. Boundary remains dry-run MCP only; Surfpool/devnet scripts are evidence helpers, no mainnet/no MCP payment/invoke.
