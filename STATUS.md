...
0. MagicBlock agent-vault B2 is merged in PR #269 (`7573f091`) and post-merge local verification passed (`npm run check:quasar:per-abi`; PER cargo tests 32/32; `npm run check:submission:claim-boundaries`). Settlement semantics spec merged in PR #271 (`aa64dcf7`): `docs/MAGICBLOCK-DELEGATED-VAULT-SETTLEMENT-SEMANTICS-2026-05-08.md`. Next: implement B3.1 raw vault `commitAndUndelegatePermission` proof first; do not introduce receipt/status writes inside TEE unless pre-delegated/prepared, and do not claim base-layer vault settlement until post-state + withdrawal evidence passes.
