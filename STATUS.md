...
- Current PR: #244 `feat: add shared Quasar instruction builders` — open. Latest local branch state adds refined Phase 6.2–12 plan, BDD retrospective-gate scenario, Phase 6.2 `/economic-demo` Quasar status card, and Phase 7 target-aware Quasar AgentAccount read/decode compatibility. Local validation passed (`npm run build`, focused decoder Jest, Quasar guards, BDD index, `git diff --check`); push after committing this slice.
