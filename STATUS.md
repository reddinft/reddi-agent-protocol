...
- Next step: observe post-merge main CI, then decide whether to keep Jupiter as an honest signed-but-devnet-rejected boundary lane, implement a local/Surfpool simulation lane, or request explicit approval for a tiny capped mainnet-beta Jupiter swap if live successful Jupiter execution is demo-critical. Keep Quasar Program Tests, quasar-readiness, bdd-index-guard, source-conformance-matrix, and Vercel as relevant evidence gates.
