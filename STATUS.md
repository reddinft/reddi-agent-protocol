...
- Local working tree: contains follow-up Quasar CI parity fix: experiment `Cargo.lock` files pinned to `proc-macro-crate 3.3.0` to avoid edition-2024 transitive crates under Solana `cargo build-sbf`/Cargo 1.79. Local full Quasar compile/test loop passes; next step is commit/push and observe GitHub checks.
