...
- Local working tree: contains one follow-up Quasar CI parity fix: `third_party/quasar/Cargo.toml` narrowed to the vendored workspace crate set after first GitHub Quasar Program Tests run failed on missing upstream workspace members. Local `cargo metadata` and full Quasar compile/test loop pass; next step is commit/push and observe GitHub checks.
