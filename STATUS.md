...
- Local working tree: contains one follow-up Quasar CI parity fix: `third_party/quasar/Cargo.toml` narrowed to the vendored workspace crate set and changed to resolver 2 for Solana `cargo build-sbf`/Cargo 1.79 compatibility. Local `cargo metadata` and full Quasar compile/test loop pass; next step is commit/push and observe GitHub checks.
