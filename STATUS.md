...
0. MagicBlock agent-vault B2 is merged in PR #269 (`7573f091`) and post-merge local verification passed (`npm run check:quasar:per-abi`; PER cargo tests 32/32; `npm run check:submission:claim-boundaries`). Settlement semantics spec merged in PR #271 (`aa64dcf7`): `docs/MAGICBLOCK-DELEGATED-VAULT-SETTLEMENT-SEMANTICS-2026-05-08.md`. B3.1 raw vault commit CPI merged in PR #273 (`e05fe8a4`): `commit_agent_vault_per` / `QPERVCMT`, with Permission/Magic Program/Magic Context pinning before vault PDA signer use and regressions for wrong permission program, wrong vault PDA, wrong magic program, and wrong magic context. Next: upgrade the devnet PER program and run a bounded MagicBlock settlement smoke that proves raw delegated vault commit post-state; do not introduce receipt/status writes inside TEE unless pre-delegated/prepared, and do not claim base-layer vault settlement until post-state + withdrawal evidence passes.
1. RAP MCP Bridge baseline and devnet lane are merged: PR #254 dry-run demo (`542e45b6`), PR #255 gated devnet MCP payments (`34c707f3`), and PR #256 hardening (`fb1dc6de`). Boundary: payment tools remain gated to Solana devnet with explicit env/approval/funder controls; no mainnet and no specialist HTTP invocation. Next: only expand MCP live invocation or mainnet behavior through a separate reviewed design/PR.
