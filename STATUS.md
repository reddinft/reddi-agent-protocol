...
3. Quasar cutover follows Issue #236, `docs/QUASAR-HACKATHON-CUTOVER-PLAN-2026-05-05.md`, and the refined staged playbook `docs/QUASAR-BDD-ITERATIVE-PLAYBOOK-2026-05-05.md`. Phase 1 inventory is scaffolded at `config/quasar/deployments.json` with `npm run check:quasar:deployments`. Candidate devnet Quasar program `VYCbMszux9seLK2aXFZMECMBFURvfuJLXsXPmJS5igW` is executable by read-only RPC check. Phase 2 guard is `npm run check:quasar:demo-readiness`; Phase 3 app/demo-agent target flag wiring merged via PR #239. Use `NEXT_PUBLIC_DEMO_PROGRAM_TARGET=quasar` / `HACKATHON_DEMO_TARGET=quasar` / `DEMO_PROGRAM_TARGET=quasar` for devnet Quasar target selection. Phase 5/6/7/8 is active in PR #244 on `feat/quasar-shared-instruction-builders-20260505`: `/register`, demo-agent registration, onboarding attestation construction, `/economic-demo` honesty UI, read/decode paths, reputation commit/reveal, and onboarding registration/confirm/dispute builders are Quasar-compatible. Runtime compatibility blockers reduced 6→4→1 after target-aware Quasar AgentAccount decoder/filter work and reputation/onboarding wrapper routing. Next loop: Phase 9 demo-agent/PER decision, then Phase 10 CI cutover → Phase 11 judge packet → Phase 12 approval-gated final validation if needed. PER still blocked. Do not deploy, sign, mutate wallets, mutate env/Coolify/Vercel, or run paid/live specialist work without explicit approval.
...
- Current PR: #244 `feat: add shared Quasar instruction builders` — open. Latest local branch state adds refined Phase 6.2–12 plan, BDD retrospective-gate scenario, Phase 6.2 `/economic-demo` Quasar status card, Phase 7 target-aware Quasar AgentAccount read/decode compatibility, and Phase 8 Quasar reputation/onboarding tx wrapper routing. Local validation passed (`npm run build`, focused Quasar Jest, Quasar guards, BDD index, `git diff --check`); push after committing this slice.
