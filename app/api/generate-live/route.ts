  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // Step 6b: Walrus off-chain storage
        let walrusBlobId = "";
        let walrusStoreFailed = false;
        try {
          const walrusResult = await storeOnWalrus(agentResponse);
          walrusBlobId = walrusResult.blobId;
          emit({
            delay: Date.now() - startTime,
            icon: "🐋",
            section: "Walrus Storage",
            lines: [
              "Storing response payload on Walrus testnet...",
              `→ Blob ID: ${walrusBlobId.slice(0, 16)}...`,
              walrusResult.alreadyCertified ? "✅ Already certified (dedup)" : "✅ Newly stored on Walrus",
              `→ Retrievable: aggregator.walrus-testnet.walrus.space/v1/blobs/${walrusBlobId.slice(0, 12)}...`,
            ],
          });
        } catch (e: unknown) {
          walrusStoreFailed = true;
          ...
          emit({
            ...
            section: "Walrus Storage",
            lines: [
              `Walrus store failed (testnet may be down): ${errMsg}`,
              "→ Continuing without off-chain storage ref",
            ],
          });
        }
...
<body>
  <div class="container">
    ...
    <div class="meta">Generated by qwen3:8b via Ollama · Reddi Agent Protocol devnet · Slot ${devnetProof.slot} · SHA256: ${responseHash.slice(0, 16)}...${walrusBlobId ? ` · Walrus: ${walrusBlobId.slice(0, 12)}...` : " · Walrus: unavailable"}</div>
  </div>
</body>
</html>`;
