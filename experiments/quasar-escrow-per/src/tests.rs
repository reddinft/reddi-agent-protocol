...
fn setup() -> QuasarSvm {
    let elf = std::fs::read(
        concat!(env!("CARGO_MANIFEST_DIR"), "/target/deploy/quasar_escrow_per_poc.so"),
    )
    .expect("build .so first: cargo build-sbf --manifest-path experiments/quasar-escrow-per/Cargo.toml");
