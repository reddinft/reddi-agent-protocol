...
Most bounty/ecosystem surfaces are incorporated into the website/demo enough to showcase, but they are not all equally strong. Quasar, x402-compatible payment boundaries, Pay.sh / `reddi-x402` sandbox charge evidence, OpenRouter, Surfpool, and Jupiter-boundary evidence are strong. Umbra is a strong planned privacy-payments lane with a direct bounty fit, but it is currently architecture/research evidence rather than executed SDK/devnet proof. Torque exists and is now clearer on the homepage. MagicBlock has moved beyond generic boundary proof into **live Quasar-native delegation proof**, but it is still **not a successful PER settlement proof**: createPermission/delegatePermission/escrow delegation succeed against MagicBlock programs, while the delegated Quasar program image fails at instruction start on MagicBlock TEE. ElizaOS/SendAI exist as framework adapter evidence, not core on-chain proof.
...
| Umbra | `docs/UMBRA-PRIVACY-PAYMENTS-BOUNTY-FIT-2026-05-07.md`, `ingests/umbra-docs-2026-05-07/` | Strong planned privacy lane, not executed proof: direct fit for private x402 payments, but no SDK/devnet smoke yet | “Umbra is the planned private-settlement adapter for Reddi Agent Protocol x402 payments; current evidence is architecture/bounty-fit analysis only.” |
