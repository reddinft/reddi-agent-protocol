...
Most bounty/ecosystem surfaces are incorporated into the website/demo enough to showcase, but they are not all equally strong. Quasar, x402-compatible payment boundaries, Pay.sh / `reddi-x402` sandbox charge evidence, OpenRouter, Surfpool, and Jupiter-boundary evidence are strong. Umbra is now a strong private-payments adapter-contract lane with a direct bounty fit: SDK packages are installed/import-verified and the mocked receiver-claimable UTXO lifecycle generates a selective-disclosure receipt, but live SDK/devnet settlement is not claimed. Torque exists and is now clearer on the homepage. MagicBlock has moved beyond generic boundary proof into **live Quasar-native delegation proof**, but it is still **not a successful PER settlement proof**: createPermission/delegatePermission/escrow delegation succeed against MagicBlock programs, while the delegated Quasar program image fails at instruction start on MagicBlock TEE. ElizaOS/SendAI exist as framework adapter evidence, not core on-chain proof.
...
| Umbra | `docs/UMBRA-PRIVACY-PAYMENTS-BOUNTY-FIT-2026-05-07.md`, `artifacts/umbra-private-x402/20260507T074334Z/SUMMARY.md`, `ingests/umbra-docs-2026-05-07/` | Strong adapter-contract proof: direct fit for private x402 payments; SDK packages installed/import-verified and mocked receiver-claimable UTXO lifecycle generates a selective-disclosure receipt. No live SDK/devnet settlement claim yet | “Umbra is the private-settlement adapter contract for Reddi Agent Protocol x402 payments; current evidence proves the SDK-facing call path and receipt shape, not live private settlement.” |
