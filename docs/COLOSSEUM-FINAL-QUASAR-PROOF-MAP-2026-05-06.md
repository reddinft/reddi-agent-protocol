...
| MagicBlock | Live boundary proof, not successful settlement claim | `docs/MAGICBLOCK-PER-TEE-VALIDATION-2026-05-07.md`; `artifacts/per-happy/20260507-090621/SUMMARY.md` shows authenticated TEE token metadata, PER-routed signature, public devnet invisibility, fallback disabled | TEE finalized with `InvalidAccountForFee`; final Quasar demo does not claim PER/TEE execution |
...
5. Show ecosystem map: x402/OpenRouter/Jupiter/Surfpool/MagicBlock boundaries. For Jupiter, say: “we can build and sign a Jupiter-routed transaction, but public Jupiter devnet execution is not supported; successful live Jupiter requires mainnet approval.” For MagicBlock, say: “we authenticated to the devnet TEE and submitted a PER-routed settlement with fallback disabled; it was not visible on public devnet RPC, but the TEE rejected it, so we do not claim successful PER settlement yet.”
6. Close honestly: final demo is Quasar-native devnet proof; not mainnet-ready; MagicBlock has live boundary proof but no successful PER settlement claim; successful live Jupiter swap is not claimed unless separately run.
