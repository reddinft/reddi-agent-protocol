...
const POST_COMMIT_WAIT_MS = Number(process.env.QUASAR_PER_POST_COMMIT_WAIT_MS || "0");
const COMMIT_RPC = process.env.QUASAR_PER_COMMIT_RPC || "base";
const COMMIT_MODE = process.env.QUASAR_PER_COMMIT_MODE || "magic-intent";
const OUT_DIR = process.env.OUT_DIR || path.join("artifacts", "quasar-per-agent-vault-settlement-smoke", new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z"));
...
const QPERVCMT = Buffer.from([81, 80, 69, 82, 86, 67, 77, 84]); // QPERVCMT commit_agent_vault_per
const QPERVMAG = Buffer.from([81, 80, 69, 82, 86, 77, 65, 71]); // QPERVMAG commit_agent_vault_magic_intent_per
const QPERVWDR = Buffer.from([81, 80, 69, 82, 86, 87, 68, 82]); // QPERVWDR withdraw_agent_vault
...
function decodeAgentVault(info) {
  ...
  return {
    ...
    balance: data.readBigUInt64LE(33).toString(),
    lifetimeCredited: data.readBigUInt64LE(41).toString(),
    lifetimeWithdrawn: data.readBigUInt64LE(49).toString(),
    status: data.readUInt8(57),
    bump: data.readUInt8(58),
  };
}
function baseVaultSettledCheck(preInfo, postInfo, amountLamports, expectedAuthority) {
  const pre = decodeAgentVault(preInfo);
  const post = decodeAgentVault(postInfo);
  const amount = BigInt(amountLamports);
  const preBalance = BigInt(pre?.balance || "0");
  const preLifetime = BigInt(pre?.lifetimeCredited || "0");
  const postBalance = BigInt(post?.balance || "0");
  const postLifetime = BigInt(post?.lifetimeCredited || "0");
  return {
    ok: Boolean(
      postInfo &&
      postInfo.owner.equals(PROGRAM_ID) &&
      post?.discriminator === 11 &&
      post?.authority === expectedAuthority.toBase58() &&
      post?.status === 0 &&
      postBalance === preBalance + amount &&
      postLifetime === preLifetime + amount
    ),
    pre,
    post,
    expectedAuthority: expectedAuthority.toBase58(),
    expectedBalanceDelta: amount.toString(),
    actualBalanceDelta: (postBalance - preBalance).toString(),
    expectedLifetimeCreditedDelta: amount.toString(),
    actualLifetimeCreditedDelta: (postLifetime - preLifetime).toString(),
    owner: postInfo?.owner.toBase58(),
  };
}
...
addresses.commitRpc = COMMIT_RPC;
addresses.commitMode = COMMIT_MODE;
...
let commitVaultResult = { ok: false, skipped: true, reason: "delegation failed" };
let commitEscrowResult = { ok: false, skipped: true, reason: "delegation failed" };
let withdrawAfterSettlementResult = { ok: false, skipped: true, reason: "settlement failed" };
let baseVaultBeforeSettlementInfo = null;
let baseVaultAfterSettlementInfo = null;
...
if (delegateEscrowResult.ok && delegateVaultResult.ok) {
  ...
  if (!releaseToVaultResult.ok && SKIP_COMMIT_ON_RELEASE_FAIL) {
    commitVaultResult = { ok: false, skipped: true, reason: "release_to_vault failed and QUASAR_PER_SKIP_COMMIT_ON_RELEASE_FAIL=1" };
    commitEscrowResult = { ok: false, skipped: true, reason: "release_to_vault failed and QUASAR_PER_SKIP_COMMIT_ON_RELEASE_FAIL=1" };
  } else {
    baseVaultBeforeSettlementInfo = await connection.getAccountInfo(vault, "confirmed");
    if (COMMIT_MODE === "magic-intent") {
      commitVaultResult = await sendCaptured(COMMIT_RPC === "tee" ? teeConnection : connection, new Transaction().add({ programId: PROGRAM_ID, keys: [
        { pubkey: agent.publicKey, isSigner: true, isWritable: true }, { pubkey: vault, isSigner: false, isWritable: true }, { pubkey: sdk.MAGIC_PROGRAM_ID, isSigner: false, isWritable: false }, { pubkey: sdk.MAGIC_CONTEXT_ID, isSigner: false, isWritable: true }, { pubkey: SystemProgram.programId, isSigner: false, isWritable: false },
      ], data: QPERVMAG }), [agent], "07-commit-agent-vault-magic-intent");
    } else {
      commitVaultResult = await sendCaptured(COMMIT_RPC === "tee" ? teeConnection : connection, new Transaction().add({ programId: PROGRAM_ID, keys: [
        { pubkey: agent.publicKey, isSigner: true, isWritable: true }, { pubkey: vault, isSigner: false, isWritable: true }, { pubkey: vaultPermission, isSigner: false, isWritable: true }, { pubkey: sdk.PERMISSION_PROGRAM_ID, isSigner: false, isWritable: false }, { pubkey: sdk.MAGIC_PROGRAM_ID, isSigner: false, isWritable: false }, { pubkey: sdk.MAGIC_CONTEXT_ID, isSigner: false, isWritable: true },
      ], data: QPERVCMT }), [agent], "07-commit-agent-vault");
    }
    baseVaultAfterSettlementInfo = await connection.getAccountInfo(vault, "confirmed");
    commitEscrowResult = await sendCaptured(COMMIT_RPC === "tee" ? teeConnection : connection, new Transaction().add({ programId: PROGRAM_ID, keys: [
    ...
    ], data: Buffer.concat([QPERCMIT, escrowIdBytes]) }), [payer], "08-commit-escrow");
  }
}

if (POST_COMMIT_WAIT_MS > 0) await sleep(POST_COMMIT_WAIT_MS);

const vaultPostCommitInfo = await connection.getAccountInfo(vault, "confirmed");
const baseVaultSettled = baseVaultSettledCheck(baseVaultBeforeSettlementInfo, vaultPostCommitInfo, AMOUNT_LAMPORTS, agent.publicKey);
if (baseVaultSettled.ok) {
  withdrawAfterSettlementResult = await sendCaptured(connection, new Transaction().add({ programId: PROGRAM_ID, keys: [
    { pubkey: agent.publicKey, isSigner: true, isWritable: true }, { pubkey: vault, isSigner: false, isWritable: true },
  ], data: Buffer.concat([QPERVWDR, u64le(AMOUNT_LAMPORTS)]) }), [agent], "09-withdraw-after-settlement");
}
...
const summary = {
  ok: Boolean(fundAgentResult.ok && createIntentResult.ok && lockResult.ok && prepareVaultResult.ok && prepareIntentResult.ok && delegateEscrowResult.ok && delegateVaultResult.ok && releaseToVaultResult.ok && commitVaultResult.ok && commitEscrowResult.ok && baseVaultSettled.ok && withdrawAfterSettlementResult.ok),
  claimBoundary: "base-layer agent-vault settlement only if baseVaultSettled.ok && withdrawAfterSettlement.ok; arbitrary-wallet private settlement remains unclaimed",
  ...
  fundAgentResult, createIntentResult, lockResult, prepareVaultResult, prepareIntentResult, delegateEscrowResult, delegateVaultResult, releaseToVaultResult, commitVaultResult, commitEscrowResult, baseVaultSettled, withdrawAfterSettlementResult, postAccounts,
};
